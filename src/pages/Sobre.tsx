        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimatedSection>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-black text-white mb-4">
              Sobre a <span className="text-gradient-red">DJ Caminhões</span>
            </h1>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <AnimatedSection animation="animate-slide-left">
              ...
              <h2 className="text-3xl sm:text-4xl font-display font-black text-white mb-6">
                Mais de <span className="text-gradient-red">15 anos</span> movendo o Brasil
              </h2>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <AnimatedSection animation="animate-slide-right">
              <div className="space-y-8">
                <div className="p-6 rounded-2xl bg-dark border border-dark-border">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-display font-black text-white mb-4">
              O que nos <span className="text-gradient-red">Move</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.map((val, i) => (
              <AnimatedSection key={val.title} className={`delay-${Math.min((i + 1) * 100, 600)}`}>
                <div className="group p-6 rounded-2xl bg-dark-card border border-dark-border hover:border-primary/30 transition-all duration-300 hover:-translate-y-1 text-center">
                  <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-primary/20 transition-colors">
                    <val.icon size={24} className="text-primary" />
                  </div>
                  <h4 className="text-white font-display font-bold text-lg mb-2">{val.title}</h4>
                  <p className="text-sm text-light-muted leading-relaxed">{val.desc}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
